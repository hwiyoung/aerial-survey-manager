...
setup_environment() {
    ...
    log_info "환경 설정 완료"
    echo ""
    echo -e "${YELLOW}=== 신규 DB 최초 로그인 설정 ===${NC}"
    echo "아래 정보는 DB에 사용자가 없을 때만 최초 계정으로 적용됩니다."
    echo "기존 DB에 사용자가 있으면 .env 값과 관계없이 기존 로그인 정보를 사용하세요."
...
print_completion() {
    ...
    echo "  1. 신규 DB는 설치 시 설정한 계정으로 로그인"
    echo "     기존 DB는 기존 계정과 비밀번호로 로그인"
