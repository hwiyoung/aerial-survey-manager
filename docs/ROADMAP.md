...
| 날짜 | 항목 | 내용 |
|------|------|------|
| 2026-02-02 | 업로드 상태 관리 | UploadProgressPanel 조건부 렌더링 (처리 옵션 화면에서만 표시), 페이지 이탈 시 자동 정리 |
| 2026-02-02 | 업로드 경고 | 업로드 중 페이지 이탈 시 경고 표시 (beforeunload, 뒤로가기, 로고 클릭 등 모든 네비게이션) |
...
### 업로드 제한 사항 (2026-02-02)
- **단일 탭 업로드 제한**: 현재 아키텍처에서는 한 브라우저 탭에서 하나의 프로젝트만 업로드 가능
- **여러 프로젝트 동시 업로드**: 여러 탭을 열어 각각 업로드해야 함
- **페이지 이탈 시 업로드 중단**: 업로드 중 페이지 이탈 시 업로드가 중단되며, 중단된 이미지는 'interrupted' 상태로 처리됨
- **향후 개선 예정**: 글로벌 업로드 기능 (페이지 이동해도 업로드 유지)

### 향후 기능 요청
- 대시보드 지도에 권역정보 표시 필요 (진행 중)
- 그룹 단위 일괄 작업 기능
- 글로벌 업로드 기능 (업로드 중 앱 내 네비게이션 허용)
