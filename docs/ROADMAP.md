...
| Phase 5: Advanced Features | ✅ 완료 | 지도 시각화, 통계, 그루핑, 정사영상 오버레이 |
...
## ⚠️ Known Issues (2026-01-21)

### COG 로딩 문제
- **현상**: 지도에서 정사영상(COG)이 로드되지 않거나 타임아웃 발생
- **원인 추정**: MinIO presigned URL이 내부 호스트명(`minio:9000`)으로 생성되어 브라우저에서 접근 불가
- **해결 방안**: 
  - `MINIO_PUBLIC_ENDPOINT` 환경변수를 모든 서비스에 설정
  - StorageService의 URL 변환 로직 개선 필요

### Bounding Box 표시 문제
- **현상**: 프로젝트 영역이 정확한 위치에 표시되지 않거나 랜덤하게 나타남
- **원인**: 정사영상 처리 완료 후 지리 좌표 추출 로직이 DB에 bounds를 저장하지 않음
- **해결 방안**:
  - 처리 워커에서 GDAL을 사용하여 COG 파일에서 bounds 추출
  - 추출된 좌표를 Project.bounds 컬럼에 자동 업데이트
