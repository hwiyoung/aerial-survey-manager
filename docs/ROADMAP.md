...
### ⚠️ Active Issues & Validation Required (진행 중)
사용자 피드백을 반영하여 다음 사항들을 중점적으로 확인 및 수정 중입니다:
1. **메타데이터 표시**: 단일 클릭 시 촬영면적/정사영상 용량 외 추가적인 메타데이터(이미지 수 등) 표시 누락 확인 필요
2. **대시보드 복귀**: 프로젝트 선택 해제 또는 로고 클릭 시 메인 대시보드 뷰로 정상 복귀 여부 재검증
3. **처리 옵션 썸네일**: 처리 설정 사이드바 내 촬영 위치 클릭 시 썸네일 팝업 노출 여부 확인
4. **처리 완료 동작**: '현재 화면 유지(Stay)' 선택 시 UI 상태가 '완료'로 정상 갱신되는지 확인
5. **지도 클릭 우선순위**: 권역(Region)과 프로젝트(Footprint) 중첩 시, 프로젝트가 확실하게 우선 선택되는지 검증 (권역 interactive: false 또는 Z-index 조정 필요)
