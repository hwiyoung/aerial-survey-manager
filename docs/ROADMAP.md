...
| 날짜 | 항목 | 내용 |
|------|------|------|
| 2026-02-02 | UI/UX 개선 | 내보내기 프로그레스바 점진적 애니메이션(5%→85%→100%), 기본 GSD 12cm로 변경, 정사영상 용량 GB 단위 표시 |
| 2026-02-02 | 지도 버그 수정 | 프로젝트 전환 시 정사영상 미표시 문제 해결 (TileLayer key prop, currentProjectIdRef 경쟁 조건 방지) |
| 2026-02-02 | 통계 수정 | 처리 완료 면적 비율 계산 수정 (분모: 대한민국 국토 면적 103,643km²) |
| 2026-02-02 | 필터링 수정 | 사이드바 권역 필터 실제 권역명과 일치하도록 수정 (9개 권역) |
| 2026-02-02 | 한글화 | Metashape 처리 진행 메시지 영문 완전 제거 (한글만 표시) |
| 2026-02-02 | 멀티 프로젝트 업로드 | 단일 탭에서 여러 프로젝트 동시 업로드 지원. 대시보드에서 전체 현황, 처리 화면에서 개별 현황 표시 |
| 2026-02-02 | 글로벌 업로드 | 앱 내 네비게이션 중 업로드 유지 가능. 업로드 패널 글로벌 표시. 브라우저 종료 시만 경고 |
| 2026-02-02 | 업로드 상태 관리 | UploadProgressPanel 조건부 렌더링 (업로드 있으면 항상 표시) |
| 2026-02-02 | 업로드 검증 | 처리 시작 전 이미지 업로드 상태 검증 추가. 불완전한 업로드 감지 시 사용자 확인 후 진행 |
| 2026-02-02 | 업로드 UX | 업로드 완료 시 실패한 이미지가 있으면 즉시 알림 표시 및 경고 배너 추가 |
| 2026-02-02 | Metashape 로그 | Alignment 결과 로깅 추가 (정렬 성공/실패 카메라 수), 로그 출력 10% 단위로 축소 |
| 2026-02-02 | Metashape 디버깅 | project.files 조건부 삭제 (alignment 95% 이상일 때만 삭제, 문제 시 보존) |
...
| 2026-01-30 | Metashape | GSD 이름 복사본(result.tif_*cm.tif) 기본 비활성화 |
...
- [x] **라이선스 안정화 전략**: Always Active + Volume Persistence(/var/tmp/agisoft/licensing) 전략 적용
- [x] **Metashape 2.2.0 호환성**: `.activated` 속성 에러 수정 및 `.valid` 대응 완료
...
| 날짜 | 항목 | 내용 |
|------|------|------|
| 2026-01-29 | Metashape 고도화 | 2.2.0 호환성 패치, 라이선스 파일 경로(/var/tmp/agisoft/licensing) 정정 및 볼륨 영속화 완료 |
...
### 글로벌 업로드 기능 (2026-02-02) ✅
- **앱 내 네비게이션 중 업로드 유지**: 업로드 중에도 대시보드, 다른 프로젝트 등으로 자유롭게 이동 가능
- **업로드 패널 글로벌 표시**: 업로드 진행 중이면 어느 화면에서든 우측 하단에 진행률 패널 표시
- **브라우저 종료/새로고침 시 경고**: 실제 페이지 이탈 시에만 경고 표시 (beforeunload)
- **멀티 프로젝트 동시 업로드**: 단일 탭에서 여러 프로젝트 동시 업로드 지원
  - 대시보드: 모든 프로젝트 업로드 현황 동시 표시 (프로젝트별 접기/펼치기)
  - 처리 옵션 화면: 해당 프로젝트의 업로드 현황만 표시
...
## Phase 9: UX Refinements & Stabilization (Current Sprint)

### 9.1 ✅ Recent Improvements (2026-01-29)
- [x] **대시보드 메타데이터 표시**: 단일 클릭으로 프로젝트 용량, 면적, 날짜 등 상세 통계 즉시 표시
- [x] **썸네일 시스템 고도화**: 업로드 완료 즉시 백엔드 썸네일 생성 트리거 및 기존 데이터 백필 스크립트 적용
- [x] **동적 상호작용**: 지도 오버랩 시 선택 팝업 및 권역보다 프로젝트 우선 선택 로직 적용
- [x] **처리 완료 UX**: 완료 팝업 내 '머물기' 선택 시 진행률 갱신 및 상태 동기화

### ✅ Resolved Issues (2026-01-29 완료)
사용자 피드백을 반영하여 다음 사항들을 수정 완료하였습니다:
1. **메타데이터 표시**: ✅ Sidebar compact 모드에 이미지 수 표시 추가
2. **대시보드 복귀**: ✅ 로고 클릭 시 페이지 리로드 없이 상태 기반 네비게이션으로 대시보드 복귀
3. **처리 옵션 썸네일**: ✅ 팝업 내 이미지 로드 실패 시 폴백 처리 추가
4. **처리 완료 동작**: ✅ 'Stay' 선택 시 processingProject 상태 동기화로 UI 즉시 갱신
5. **지도 클릭 우선순위**: ✅ 프로젝트 Rectangle 이벤트 전파 완전 차단 + RegionBoundaryLayer에서 프로젝트 영역 클릭 무시

### 9.2 ✅ 추가 버그 수정 (2026-01-29 저녁)
- [x] **이미지 수 중복 오류**: 같은 파일명 업로드 시 Image 레코드 중복 생성 방지 (`upload.py` 중복 체크 로직 추가)
- [x] **썸네일 즉시 표시**: 업로드 완료 후 3초/8초 지연 새로고침으로 Celery 썸네일 생성 결과 반영
- [x] **처리 완료 무한 깜빡임**: `onComplete` 호출을 useEffect에서 모달 버튼 핸들러로 이동
- [x] **'나중에 확인' 버튼 미작동**: 버튼 클릭 핸들러에 `e.stopPropagation()` 추가
- [x] **브라우저 뒤로가기**: `popstate` 이벤트 리스너 추가로 히스토리 네비게이션 지원
- [x] **프로젝트 생성 오류**: `newProject is not defined` 변수명 오류 수정 (`created` → 올바른 참조)
- [x] **업로드 패널 자동 숨김**: 처리 시작 시 `setActiveUploads([])` 호출로 패널 자동 닫힘

### 9.3 ✅ 툴팁 우선순위 수정 완료 (2026-01-29 야간)
- [x] **권역 툴팁 vs 바운딩박스 툴팁 우선순위**: 다중 레이어 접근 방식으로 해결
  - `hoveredProjectId` 상태를 `RegionBoundaryLayer`에 전달
  - `useEffect`로 프로젝트 호버 시 모든 권역 레이어 툴팁 강제 닫기
  - `tooltipopen` 이벤트에서 마우스 위치에 프로젝트가 있는지 확인 후 조건부 닫기
  - `mouseover` 핸들러에서 프로젝트 영역 내 마우스일 경우 툴팁 열기 방지
  - CSS 규칙 `.project-hovered .region-tooltip { display: none !important; }` 추가
  - Rectangle에 `bubblingMouseEvents: false` 설정으로 이벤트 전파 차단
  - 지도 컨테이너에 `project-hovered` 클래스 동적 추가

---

## 최근 변경 이력 업데이트 (2026-01-29)

| 날짜 | 항목 | 내용 |
|------|------|------|
| 2026-01-29 | 툴팁 우선순위 | ✅ 권역 레이어 켜진 상태에서 바운딩박스 호버 시 권역 툴팁 숨김 완료 (다중 레이어 접근) |
| 2026-01-29 | 버그 수정 | 이미지 중복 방지(upload.py), 썸네일 지연 새로고침, 처리 완료 깜빡임 수정, 뒤로가기 지원 |
| 2026-01-29 | 지도 UX | 바운딩박스/권역 툴팁 우선순위 로직 개선 (layersRef + tooltipopen 이벤트 제어) |
| 2026-01-29 | Phase 9 완료 | Active Issues 5개 항목 전체 수정: 메타데이터 표시, 대시보드 복귀, 썸네일 폴백, 상태 동기화, 지도 클릭 우선순위 |
| 2026-01-29 | UI 고도화 | 중첩 프로젝트 선택 팝업(Overlap Menu) 구현, 썸네일 생성 파이프라인 자동화 |
| 2026-01-29 | 메타데이터 | 대시보드 통계 카드 데이터 소스 정상화 (null check 및 필드명 매핑 수정) |
| 2026-01-29 | 안정성 | Frontend 에러(Tooltip reference) 긴급 수정 및 재배포 |
| 2026-01-29 | Metashape 고도화 | 2.2.0 호환성 패치, 라이선스 파일 경로(/var/tmp/agisoft/licensing) 정정 및 볼륨 영속화 완료 |
| 2026-01-27 | Metashape 통합 | Airflow 의존성 제거 및 Celery 워커 기반 통합 완료, 라이선스 자동 활성화 대응 |
