...
### ✅ Phase 7: UX Refinement & Stabilization
...
- [ ] **메타데이터 표시 확장**: 이미지 수, GSD 등 추가 정보 표시
- [ ] **대시보드 복귀 UX**: 로고 클릭 및 선택 해제 시 대시보드 뷰 복귀 검증
- [ ] **썸네일 가시성**: 처리 옵션 내 썸네일 노출 및 팝업 동작 확인
- [ ] **처리 완료 상태 동기화**: '머물기' 동작 시 UI 상태 즉시 갱신 확인

## ⚠️ Known Issues (2026-01-29)

### 지도 및 상호작용
- **클릭 우선순위**: 권역(Region)과 프로젝트(Footprint) 중첩 시 간헐적으로 권역이 먼저 선택되는 현상 (Interactive false 적용 필요)
- **대시보드 복귀**: 일부 상황에서 메인 대시보드로 돌아가는 네비게이션이 동작하지 않음

### 썸네일 및 미디어
- **처리 옵션 썸네일**: 처리 설정 창에서 촬영 위치 포인트 클릭 시 썸네일이 나타나지 않는 경우 발생
- **메타데이터 표시**: 일부 프로젝트에서 메타데이터(이미지 수 등)가 누락되는 현상

### 시스템
- **COG Loading**: MinIO presigned URL 외부 접근 시 `MINIO_PUBLIC_ENDPOINT` 설정 필요
- **권역 시각화**: 폴리곤 과다로 인한 렌더링 지연 (Canvas 단순화 적용 중)
