...
def make_args():
    ...
    parser.add_argument("-i", "--dataset_path", type=str, default="workspace/input", help="input folder containing T1/, T2/")
    parser.add_argument("-m", "--model_path", type=str, default="workspace/model", help="Model folder containing .onnx")
    parser.add_argument("-o", "--output_path", type=str, default="workspace/output", help="Output folder")
...
def resolve_paths(args):
    t1_path = os.path.join(args.dataset_path, "T1")
    ...
    t2_path = os.path.join(args.dataset_path, "T2")
    ...
    model_files = [f for f in os.listdir(args.model_path) if f.endswith(".onnx")]
    if not model_files:
...
def main():
    ...
    status = StatusManager(args.output_path)
    ...
    with status.task("Loading ONNX Model"):
        ...
        session, config = create_session(get_model_file(args.model_path), max_threads=args.max_threads)
    ...
    with status.task("Classification"):
        ...
        result.to_file(os.path.join(args.output_path, "result.json"), driver="GeoJSON", encoding="euc-kr")
