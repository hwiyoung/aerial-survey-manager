...
cleanup() {
    ...
    # 라이센스 비활성화 스크립트 실행
    if [ -f "/app/engines/metashape/dags/metashape/deactivate.py" ]; then
        python3 /app/engines/metashape/dags/metashape/deactivate.py
        DEACTIVATE_RESULT=$?
        ...
        echo "[Entrypoint] deactivate.py 파일을 찾을 수 없습니다"
    fi
